...
class R2Uploader:
    ...
    def __init__(self):
        account_id = os.environ["CLOUDFLARE_ACCOUNT_ID"].strip().replace('﻿', '')
        access_key = os.environ["CLOUDFLARE_R2_ACCESS_KEY_ID"].strip().replace('﻿', '')
        secret_key = os.environ["CLOUDFLARE_R2_SECRET_ACCESS_KEY"].strip().replace('﻿', '')
        self.bucket = os.environ["CLOUDFLARE_R2_BUCKET_NAME"].strip().replace('﻿', '')
        self.public_url_base = os.environ.get("CLOUDFLARE_R2_PUBLIC_URL", "").strip().replace('﻿', '').rstrip("/")
