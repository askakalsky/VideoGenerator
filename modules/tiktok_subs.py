...
class SubtitleStyle:
    ...
    def validate(self):
        ...
        if not 1 <= self.alignment <= 9:
            raise ValueError(f"alignment должен быть 1-9, получено: {self.alignment}")
        
        if self.font_scale <= 0 or self.font_scale > 1:
            raise ValueError(f"font_scale должен быть 0-1, получено: {self.font_scale}")
        
        # Валидация цветов
        for color_name in ['highlight_color', 'normal_color', 'outline_color', 'shadow_color']:
            color = getattr(self, color_name)
            if not self._is_valid_color(color):
                raise ValueError(f"{color_name} должен быть в формате #RRGGBB, получено: {color}")
...
class VideoInfo:
    ...
    def _load_info(self):
        ...
        try:
            ...
            raise FFmpegError(f"Не удалось получить информацию о видео: {e}") from e
...
class Transcriber:
    ...
    def load_model(self):
        ...
        try:
            ...
            raise TranscriptionError(f"Не удалось загрузить модель: {e}") from e
    ...
    def transcribe(self, media_path: Union[str, Path]) -> Any:
        ...
        logger.info(f"   └─ VAD: {'включен' if self.config.vad else 'выключен'}")
...
class SubtitleGenerator:
    ...
    def _create_style(self) -> SSAStyle:
        ...
        font_size = max(12, int(round(self.video_height * self.style.font_scale)))
        margin_v = max(0, int(round(self.video_height * self.style.margin_v_ratio)))
        margin_l = max(0, int(round(self.video_width * self.style.margin_lr_ratio)))
        margin_r = margin_l
        outline = max(1, int(round(self.video_height * self.style.outline_ratio)))
    ...
    def generate(self, transcription_result: Any, output_path: Union[str, Path]):
        ...
        for seg_idx, segment in enumerate(segments):
            ...
            if not words:
                ...
                text = escape_ass_text(safe_getattr(segment, 'text', '').strip())
            ...
            for i, timed_word in enumerate(timed_words):
                ...
                parts = [f"{{\\c&H{normal_bgr}&}}"]  # Устанавливаем базовый цвет
...
class SubtitleBurner:
    ...
    def burn(
        ...
        try:
            ...
            raise FFmpegError(f"Ошибка ffmpeg при прожиге субтитров: {e}") from e
